...
| `src/auth/jwt-claims.ts` | Client-side JWT claim decode — ID token **and** access token | `decodeJwtPayload`, `extractOrgIdFromClaims`, `extractProfile`, `subFromAccessToken`; `Profile` | No signature verification (server verifies). Returns `null`/empty on malformed input — must not throw. PDEV-7684 merged the former `src/auth/jwt.ts` + `src/utils/jwt.ts` here; the surviving decoder pads **and** UTF-8-decodes, so non-ASCII claims (names) survive. |
...
- **Decode ≠ verify.** `jwt-claims.ts` decodes without signature verification (server verifies). Never make a trust/authorization decision client-side on a decoded claim; `sub` is only a server-checkable hint.
