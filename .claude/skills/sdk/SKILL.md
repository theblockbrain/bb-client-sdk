...
Declared in `package.json` `"exports"` → source dir. **13 JS entry points** (plus the CSS asset subpath, which the contract test skips). The two newest — `./analytics` and `./analytics/mixpanel` — are on `main` but **not in a published release yet** (last tag `v0.17.0` predates them). The public-API contract test derives its list from `package.json#exports` and snapshots every entry point.
...
| 5 | **Public-API contract test** | (runs inside step 4) | `src/public-api.contract.test.ts` snapshots exports (values **and** types) of all current entry points (13 today). An undeclared surface change fails here. To change the API **intentionally**: update the snap in the same PR (`npx vitest -u`), treat as a breaking change, bump semver, and canary first. |
