...
Package: `@theblockbrain/bb-client-sdk` · v0.18.0 · `"type": "module"` (**ESM-only**) · published to **GitHub Packages** (private, `https://npm.pkg.github.com`, `access: restricted`). Runtime dep: `marked` only. Optional peers (React consumers only): `react ^19`, `@tanstack/react-query ^5`.
...
Declared in `package.json` `"exports"` → source dir. **19 JS entry points** (plus the CSS asset subpath, which the contract test skips). Six of them — `./agentic`, `./analytics`, `./analytics/mixpanel`, `./i18n`, `./media` and `./telemetry` — postdate `v0.17.0` and first reach consumers in **`0.18.0`**. The public-API contract test derives its list from `package.json#exports` and snapshots all 19.
...
|---|---|---|---|
| 1 | ms-outlook-addin | React, Office.js webview | **Reference adopter**, pins `^0.17.0` — one era behind since `0.18.0`; bumping it is the open follow-up. Canary target. |
