...
> **Invariant A — framework-agnostic core.** The core entry points (`./api`, `./auth`, `./settings`, `./utils`, `./adapters`, `./config`, `./prompt`, `./actions`) MUST stay React-free and, where possible, DOM-free. Only `./react` and `./ui` may import React. `./api` and `./auth` must tree-shake with **zero React in the graph** (`publint` + `attw` verify the export map).
>
...
Declared in `package.json` `"exports"` → source dir. **11 JS entry points ship today** (plus the CSS asset subpath, which it skips); **`./analytics` is planned** (telemetry workstream — not yet in `package.json#exports`; the row below is marked), bringing the target to 12. The public-API contract test snapshots the current entry points.
...
| `./utils` | `src/utils/` | `extractJson` (returns `T\|null`, **never throws**) + `repairUnescapedQuotes`; `subFromAccessToken`; `createLock`; `extractCode` | **All** runtimes |
| `./adapters` | `src/adapters/` | **Type-only.** `IdentityAdapter {getRedirectUri, launchOAuthFlow}`, `StorageAdapter {get, set, remove}` — the injection seams that keep surfaces thin. *(Planned: `AnalyticsAdapter` + `AnalyticsEventMap`/`AnalyticsEventName`/`AnalyticsEventProps`/`AnalyticsIdentity`/`AnalyticsErrorContext` arrive with the telemetry seam.)* | **All** runtimes (each surface implements them) |
| `./analytics` *(planned — not yet exported)* | `src/analytics/` | Runtime side of the `AnalyticsAdapter` seam: `setAnalyticsAdapter`/`getAnalyticsAdapter`/`resetAnalyticsAdapter`, the safe sink `trackEvent`/`captureError`/`trackApiError`/`flushAnalytics` (no-op when no adapter is registered, never throws into the caller), and re-exports the analytics types from `./adapters`. Type-only imports → React-free + DOM-free (invariants A + B). | **All** runtimes (each surface registers an adapter) |
...
| `./prompt` | `src/prompt/` | `parse-response.ts` — LLM output parsing | **All** runtimes |
| `./actions` | `src/actions/` | `runner.ts`, `schema.ts` — an action runner | **All** runtimes |
| `./ui` | `src/ui/` | **React-only.** `useTheme`, `ThemeToggle`, `markdown.ts` (marked), `theme.ts`, `time.ts`; class-strategy dark mode | React surfaces only |
| `./react` | `src/react/` | **React-only.** React Query data layer: `provider.tsx`, `keys.ts` (`bbKeys`), `queries.ts`, `mutations.ts`, `use-chat-stream.tsx` (`useChatStream`). Test-first; see `src/react/STATUS.md` | React surfaces only |
...
- **C — Verify all adapters.** Every change is checked against the adapter matrix (runtime/framework/auth-flow/storage/CSP): does it break any? A public-export change must pass the contract test **and** be canary-tested in a consumer (Outlook) BEFORE `latest`. The Outlook stale `^0.7.3` pin (vs 0.17.0) is the cautionary tale of silent semver fan-out. → [references/adapters.md](references/adapters.md), [references/cross-adapter-safety.md](references/cross-adapter-safety.md).
...
|---|---|---|---|
| 1 | ms-outlook-addin | React, Office.js webview | **Reference adopter** but pins stale `^0.7.3`. Canary target. |
...
| 5 | **Public-API contract test** | (runs inside step 4) | `src/public-api.contract.test.ts` snapshots exports (values **and** types) of all current entry points (11 today; 12 once `./analytics` lands). An undeclared surface change fails here. To change the API **intentionally**: update the snap in the same PR (`npx vitest -u`), treat as a breaking change, bump semver, and canary first. |
...
| [references/telemetry-release-gate.md](references/telemetry-release-gate.md) | Invariant E in depth — the planned `AnalyticsAdapter` seam (WS9), the typed `AnalyticsEventMap` taxonomy + identity model, and the release-gate Definition of Done each surface wires. |
