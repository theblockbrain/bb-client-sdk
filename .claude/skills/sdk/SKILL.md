...
> **Invariant A — framework-agnostic core.** The core entry points (`./api`, `./auth`, `./settings`, `./text`, `./utils`, `./adapters`, `./config`) MUST stay React-free and, where possible, DOM-free. Only `./react` and `./ui/react` may import React (`./ui` itself is React-free since PDEV-7724). `./api` and `./auth` must tree-shake with **zero React in the graph** (`publint` + `attw` verify the export map).
>
> Practical rule for **non-React consumers** (Slack, Lit): importing the **specific subpath** (`@theblockbrain/bb-client-sdk/api`) is still the recommended shape, but as of **PDEV-7724 the root barrel `"."` is React-free too** — `useTheme` moved to `./ui/react`, so `src/index.ts` → `./ui` no longer pulls React in. `./react` and `./ui/react` are intentionally NOT in the root barrel. Before 7724 the barrel was unimportable without React, which is what `bb-slack-integrations` hit.
>
> **Invariant A applies to types as well as runtime.** A public declaration must not name a platform-specific ambient type, because a consumer whose `lib` excludes it fails to typecheck a declaration it never calls. `Storage`, `Document`, `Element`, `DocumentFragment` and `HTMLElement` are **DOM-lib only** — use a structural interface (`WebStorageArea`, `MarkdownDocument`, `OfficeGlobal` are the precedents). `URL`, `AbortSignal`, `Blob`, `File` and `FormData` are declared by `@types/node` and are safe to name; replacing those with look-alikes loses fidelity for no gain. `check:cleanroom` phase 3 enforces this by typechecking the packed tarball as a DOM-less Node consumer.

---

## 2. Layer map — entry points

Declared in `package.json` `"exports"` → source dir. **21 JS entry points** (plus the CSS asset subpath, which the contract test skips). Six of them — `./agentic`, `./analytics`, `./analytics/mixpanel`, `./i18n`, `./media` and `./telemetry` — postdate `v0.17.0` and first reach consumers in **`0.18.0`**; `./dev/sdk-link` arrived in `0.19.0` and `./analytics/faro` is unreleased (see the `0.20.0` section of the CHANGELOG). The public-API contract test derives its list from `package.json#exports` and snapshots all 21.
...
| `./adapters` | `src/adapters/` | **No longer type-only** (PDEV-7724): carries the runtime halves of the host ports — `createWebStorageAdapter`, `createWebCryptoAdapter` + `setCryptoAdapter`/`getCryptoAdapter`, `createHostCapabilityRegistry` + `routeToolCall`, `setFlagAdapter`/`isFeatureEnabled`. Still React-free and DOM-free at import time. `IdentityAdapter {getRedirectUri, launchOAuthFlow}`, `StorageAdapter {get, set, remove}` — the injection seams that keep surfaces thin. Also `AnalyticsAdapter` + `AnalyticsIdentity`/`AnalyticsErrorContext` (the telemetry seam) and `AnalyticsEventMap`/`AnalyticsEventName`/`AnalyticsEventProps`, which from `0.20.0` are `@deprecated` aliases of `CoreEventMap`/`CoreEventName`/`CoreEventProps` in `./telemetry` — prefer those. | **All** runtimes (each surface implements them) |
...
| `./analytics` | `src/analytics/index.ts` | Runtime side of the `AnalyticsAdapter` seam: `setAnalyticsAdapter`/`getAnalyticsAdapter`/`resetAnalyticsAdapter`, the safe sink `trackEvent`/`captureError`/`trackApiError`/`flushAnalytics` (no-op when no adapter is registered, never throws into the caller), the guarded identity binders `identifyUser`/`setAnalyticsGroup` (process-wide — a multi-tenant server adapter omits `identify`/`group` so they no-op), `createCompositeAdapter`/`CompositeChild` (fan one event out to several sinks, each child guarded **individually** so one throwing sink cannot silence the rest — ⚠️ it declares `identify`/`group` when **any** child does, which re-arms the process-wide binding a Slack-style adapter deliberately declined), and re-exports the analytics types from `./adapters`. Type-only imports → React-free + DOM-free (invariants A + B). | **All** runtimes (each surface registers an adapter) |
| `./analytics/mixpanel` | `src/analytics/mixpanel.ts` | **Opt-in leaf.** `createMixpanelAdapter(client, config)` — super-props, tenant grouping and a consent gate; the PII/secret scrub now lives in the shared `src/analytics/scrub.ts` (`scrubProps`) so the Faro leaf applies the identical one. `MixpanelClient` is a **structural** type over the `mixpanel-browser` methods used, so the SDK takes **no** analytics dependency and the core only pulls this in if a surface imports the subpath. | Surfaces that chose Mixpanel (they own the `mixpanel-browser` instance) |
| `./analytics/faro` | `src/analytics/faro.ts` | **Opt-in leaf, unreleased (0.20.0).** `createFaroAdapter(faro, config)` — the browser-RUM half: Mixpanel answers "are people using it", Faro answers "is it slow or broken", and the release gate wants both. Stringifies the scrubbed props to Faro's `Record<string,string>` attributes, namespaces them under a `domain` (default `blockbrain`), and re-sends user + tenant together on every change because Faro's `setUser` **replaces** rather than merges. `FaroLike` is a **structural** type over four `@grafana/faro-web-sdk` methods → no dependency added. **Browser only** — Faro has no React Native support; blocky-mobile uses Sentry RN instead. | Browser surfaces (add-ins, SPFx, SPAs, web component) |
...
- **C — Verify all adapters.** Every change is checked against the adapter matrix (runtime/framework/auth-flow/storage/CSP): does it break any? A public-export change must pass the contract test **and** be canary-tested in a consumer (Outlook) BEFORE `latest`. Outlook once sat on a `^0.7.3` pin while the SDK was at 0.17.0 — ten minor eras of silent semver fan-out, the cautionary tale. Keeping it within one era is what keeps it a usable canary; `npm run release:status` is the check. → [references/adapters.md](references/adapters.md), [references/cross-adapter-safety.md](references/cross-adapter-safety.md).
- **D — Security in every layer.** Tokens never logged (scrub `BBApiError.responseBody`), only in secure storage via the adapter, never in a bundle or `NEXT_PUBLIC_` var. PKCE S256 + state/CSRF + redirect allowlist + nonce. Refresh single-flight (`createRefreshGuard`). HTTPS + audience pinning (`OAUTH_BACKEND_URL` hardcoded). Multi-tenant: `orgId` vs `targetOrgId` discipline, **0 cross-tenant leakage**. `extractJson` never throws; sanitize markdown (XSS). Minimal deps + restricted registry + least-scope PAT + secret-scan. **Never echo untrusted text into rendered or agent-facing output** — server- and agent-controlled strings (an SSE frame's `toolId`, `responseBody`, a server error message) must not be interpolated raw into anything rendered or fed back to a model: server text can echo a submitted credential, and agent text can inject instructions or line breaks into the next turn. `describeBBApiError` ignores the body and the message by design; `routeToolCall` quotes `toolId` via `JSON.stringify` after collapsing control/format characters and clamping by **code point** (slicing UTF-16 units can split a surrogate pair). Do **not** HTML-escape at this layer — these are plain-text messages and escaping belongs at the sink. → [references/security.md](references/security.md).
- **E — Instrument every surface (HARD release gate).** NOTHING ships to production without BOTH (1) product analytics (Mixpanel: activation/funnel/retention; Zitadel `sub` as distinct id, org as group, **no PII**) AND (2) health telemetry (Sentry + Grafana Faro RUM: crash-free + error rate). The SDK exposes the **`AnalyticsAdapter`** seam (peer of Storage/Identity) with a typed event taxonomy — **one** vocabulary, `CoreEventMap` from `./telemetry` (`sign_in_completed`/`sign_in_failed`, `message_sent`, `stream_started`/`message_first_token`/`message_completed`/`stream_dropped`, `api_error{status_code,endpoint}`); `AnalyticsEventMap`/`AnalyticsEventName` are `@deprecated` aliases of it from `0.20.0`, kept so `0.19.0` type references resolve. **The seam ships in `0.18.0`** (WS9 — PDEV-6854/6855): register an adapter at surface startup via `setAnalyticsAdapter` (from `@theblockbrain/bb-client-sdk/analytics`) implementing the `AnalyticsAdapter` type (from `@theblockbrain/bb-client-sdk/adapters`), or take the ready-made `createMixpanelAdapter` from `@theblockbrain/bb-client-sdk/analytics/mixpanel` and `createFaroAdapter` from `.../analytics/faro`, composed with `createCompositeAdapter` so both halves of the gate register through the one seam. The SDK emits the auth funnel (`src/auth/login.ts`, PDEV-6855 — `sign_in_*` plus identity binding on success), the session-refresh pair (`src/auth/refresh-singleton.ts`, from inside the single-flight guard so it counts refreshes and not waiters), and the whole turn/stream funnel (`src/api/messages.ts` + `src/api/stream-result.ts`); see `references/telemetry-release-gate.md` §1. **`api_error` via `trackApiError` is the one group still unwired.** Shipping the seam did not lift the gate — it made it enforceable: each surface STILL must register an adapter forwarding to Mixpanel + Sentry/Faro, and that remains a release-gate checklist item on every surface. → [references/telemetry-release-gate.md](references/telemetry-release-gate.md).
...
|---|---|---|---|
| 1 | ms-outlook-addin | React, Office.js webview | **Reference adopter** and canary target. `npm run release:status` prints its live pin and flags it when it falls >1 minor era behind (SLO E3). |
...
| 5 | **Public-API contract test** | (runs inside step 4) | `src/public-api.contract.test.ts` snapshots exports (values **and** types) of all current entry points (21 today). An undeclared surface change fails here. To change the API **intentionally**: update the snap in the same PR (`npx vitest -u`), treat as a breaking change, bump semver, and canary first. |
...
- **No `any`, typed errors:** every `./api` call throws `BBApiError` (`statusCode`, `endpoint`, `responseBody`) on non-2xx — handle with `isBBApiError(err)` / `err.statusCode` (README: `401` → re-auth). Prefer `describeBBApiError(err)` over a hand-rolled status ladder, and never encode a domain condition in an HTTP status the application does not exclusively own — a proxy or mesh emits the same codes. Never log `responseBody` raw (may carry tokens — Invariant D).
...
| [sdk-release](../sdk-release/SKILL.md) | Cutting a version, running the canary flow, or reasoning about the publish gate. Since PDEV-7001 `publish.yml` re-runs the **full** gate on the tag plus a tag ↔ `package.json` version guard (SLO E2 closed) — what it still cannot prove is that a **consumer** works, which is why the canary stays mandatory. |
...
| [references/release-and-versioning.md](references/release-and-versioning.md) | Release + versioning in depth — semver policy for the public API, the canary flow, the publish gate (full since PDEV-7001), and the version-pin fan-out risk per consumer. |
