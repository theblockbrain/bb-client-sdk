...
| **RN-safe / pluggable transport seam** | Core `./api` calls native `fetch(` directly (e.g. `src/api/messages.ts`, `bots.ts`, `conversations.ts`); streaming parses `ReadableStream<Uint8Array>` in `src/api/blocky-sse.ts`. `sendMessage` threads `options.signal` to BOTH branches as of PDEV-7011 — Blocky since PDEV-7340, agentic since the cancellation fix (the agentic branch silently dropped it before, so an agent turn was uncancellable and a user's stop was booked as a completed success). `src/react/use-chat-stream.tsx:161` still has its enable point commented: `// signal: controller.signal, // ← enable once the SDK threads AbortSignal (WS2)` — the SDK half is done, so **that line is now the only thing between `useChatStream().stop()` and a real abort**. | `blocky-mobile` (RN `fetch`/`ReadableStream` unstable → no streaming), plus **true cancellation everywhere** (today `useChatStream().stop()` is best-effort — bumps a run-id, request keeps running; see `docs/react-layer.md` gap #2) | WS2 / WS7 |
| **`AnalyticsAdapter` seam** | **Seam released in `0.18.0`; the vocabulary rename + the Faro leaf are unreleased (`0.20.0`).** Types exported from `./adapters` (`src/adapters/analytics.ts`: `AnalyticsAdapter` + `AnalyticsIdentity`/`AnalyticsErrorContext`; `AnalyticsEventMap`/`AnalyticsEventName` are now `@deprecated` aliases of `CoreEventMap`/`CoreEventName` in `./telemetry`), runtime sink at the **`./analytics`** entry point (`src/analytics/index.ts`: `setAnalyticsAdapter`/`trackEvent`/`trackApiError`/`identifyUser`/`setAnalyticsGroup`/`createCompositeAdapter`/…), plus the opt-in **`./analytics/mixpanel`** and **`./analytics/faro`** leaves (`createMixpanelAdapter`/`createFaroAdapter`, both structurally typed — no dependency added). The SDK core still imports no `Mixpanel`/`Sentry`/`Faro`. Wired call sites: `src/auth/login.ts` (PDEV-6855 — the `sign_in_*` funnel + identity binding), `src/auth/refresh-singleton.ts` (`session_token_*`), and `src/api/messages.ts` + `src/api/stream-result.ts` (the `message_*`/`stream_*` funnel). `api_error` is the one group still unwired. ⚠️ **bb-slack-integrations** (one Node process, many orgs) must omit the process-wide `identify`/`group` from its adapter and rely on per-event identity — and must NOT compose it via `createCompositeAdapter` with a child that implements them, because the composite declares each method when *any* child does. ⚠️ **blocky-mobile**: Faro has no React Native support; use Sentry RN for the health half. | **Every surface** — makes SLO "Part B" telemetry measurable once wired (invariant **E**). The seam emits the standard event taxonomy (`sign_in_completed`/`sign_in_failed`, `message_sent`, `stream_started`/`message_first_token`/`message_completed`/`stream_dropped`, `api_error{status_code,endpoint}` — the typed `CoreEventMap` in `references/telemetry-release-gate.md` §1–2 is the canonical source of truth; renamed in `0.20.0`, old→new in `LEGACY_EVENT_RENAMES`). | Landed (WS9); per-surface wiring outstanding |
